...
private fun resolvePassword(o: Options): String? {
    // Highest precedence: stdin, then env name, then file, then literal/env.
    if (o.passwordStdin) {
        // Prompt to STDERR if a TTY/console is present
        System.console()?.let { console ->
            System.err.print("Password: ")
            System.err.flush()
            return String(console.readPassword())
        }
        // Non-interactive (launcher writes one line to stdin)
    ...
    return o.passwordLiteral // may be null (e.g., passwordless DB or URL contains creds)
}
...
private fun addPingTool(server: Server) {
    server.addTool(
        ...
        inputSchema = Tool.Input(), // no params
        outputSchema = null,
    ) {
        CallToolResult(
...
private fun addListSchemasTool(
    ...
    server.addTool(
        ...
        try {
            ...
            CallToolResult(
                content = names.map { TextContent(it) }, // List<Content>
                isError = false,
            )
        } catch (ex: Exception) {
