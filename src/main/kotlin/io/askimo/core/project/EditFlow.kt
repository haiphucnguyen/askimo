...
class EditFlow(
    ...
    fun run(instruction: String, meta: ProjectMeta) {
        // Collect files mentioned
        val sources = resolveFiles(meta, instruction)
        if (sources.isEmpty()) {
            println("ℹ️  No eligible files to edit under project root."); return
        }
        ...
        val diff = try { diffGen.generateDiff(req) } catch (e: Exception) {
            println("❌ Model error: ${e.message}"); return
        }
        if (diff.isBlank()) { println("ℹ️  No changes proposed."); return }

        val sum = DiffInspector.summarize(diff)
        if (sum.changedFiles > budgets.maxFiles || sum.totalChanged > budgets.maxChangedLines) {
            println("⛔ Exceeds budget: ${sum.changedFiles} files, ${sum.totalChanged} lines (max ${budgets.maxFiles}/${budgets.maxChangedLines}).")
            return
        }
        if (DiffInspector.containsBlockedPaths(diff)) {
            println("⛔ Diff touches guarded files (build/lock/CI)."); return
        }
        ...
        if (!yes) { println("🛑 Aborted."); return }

        if (!budgets.allowDirty && Git.isDirty(meta.root)) {
            println("⛔ Working tree dirty. Commit/stash or allow dirty."); return
        }

        val branch = "askimo/change-${TimeUtil.shortStamp()}"
        val backupPath = Paths.get(meta.root, ".askimo", "patches")
            .resolve("${TimeUtil.shortStamp()}-${Git.headShort(meta.root)}.patch")
        try { Files.createDirectories(backupPath.parent); Files.writeString(backupPath, diff) } catch (_: Exception) {}
    ...
    private fun resolveFiles(meta: ProjectMeta, instruction: String): List<SourceFile> {
        val absPaths = EditIntentDetector
            .detect(instruction, true).targetPaths
        ...
        for (absStr in absPaths) {
            ...
            if (!PathGuards.isUnderRoot(abs, meta.root)) { println("⛔ Outside root: $abs"); continue }
            if (!Files.isRegularFile(abs)) { println("⚠️ Not a file: $abs"); continue }
            if (PathGuards.isBlocked(abs)) { println("⛔ Guarded file: $abs"); continue }
    ...
    private fun rootRel(root: String, abs: Path): String =
        Paths.get(root).relativize(abs).toString().replace('\\','/')
}
