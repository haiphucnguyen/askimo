...
object EditIntentDetector {
    private val verbs = listOf(
        "add", "write", "update", "fix", "insert", "append",
        "document", "javadoc", "docstring", "comment", "refactor", "rename"
    )
    ...
    fun detect(prompt: String, hasActiveProject: Boolean): Intent {
        val p = prompt.lowercase()
        val mentionsVerb = verbs.any { v -> p.contains(" $v ") || p.startsWith("$v ") || p.contains("$v to ") }
        val paths = pathToken.findAll(prompt)
            .map { it.value }
